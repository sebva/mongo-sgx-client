...
void MongoDatabase::mongo_bootstrap() {
    ...
    bson_error_t error;
    mongoc_uri_t *uri =
        mongoc_uri_new_with_error(connection_url.c_str(), &error);
    if (!uri) {
...
#pragma region First index, guarantee unicity of names
    bson_t *command = BCON_NEW(
        "createIndexes", BCON_UTF8(COLLECTION_NAME), "indexes", "[", "{", "key",
        "{", "name", BCON_INT32(1), "}", "name", "user_name_unique", "unique",
        BCON_BOOL(true), "}", "{", "key", "{", "groups", BCON_INT32(1), "}",
        "name", "groups_performance", "}", "]");
